...
class Placement:
    ...
    def placement_num(self,excel_path):
        ...
        for i in range(2,50):
            try:
                df = pla.cell(row=i, column=1).value
                if df is None:
                    num = i - 2
                    break
    ...
    def beginTime(self,excel_path):
        ...
        for i in range(2,planum+2):
            try:
                df = pla.cell(row=i,column=4).value
    ...
    def expireTime(self,excel_path):
        ...
        for i in range(2, planum + 2):
            try:
                df = pla.cell(row=i, column=5).value
                dt = datetime.strptime(df, "%Y %m-%d %H:%M")
                timestamp_seconds = dt.timestamp()
                timestamp_milliseconds = int(timestamp_seconds * 1000)
    ...
    def itemId(self,excel_path):
        ...
        for i in range(2, planum + 2):
            try:
                df = int(pla.cell(row=i, column=1).value)
        ...
        for i in range(len(unlocktype)):
            try:
                if unlocktype[i] == 'true':
                    df = pla.cell(row=i+2, column=8).value
                    if df is None:
    ...
    def needUnlock(self,excel_path):
        ...
        for i in range(2, planum + 2):
            try:
                df = float(pla.cell(row=i, column=6).value)
                if df == 1:
    ...
    def sendType(self,excel_path):
        ...
        for i in range(2, planum + 2):
            try:
                df = pla.cell(row=i, column=9).value
                if df is None:
    ...
    def totalCount(self,excel_path):
        ...
        for i in range(2, planum + 2):
            try:
                df = pla.cell(row=i, column=3).value
                if df is None:
    ...
    def unlockType(self,excel_path):
        ...
        for i in range(len(unlocktype)):
            try:
                if unlocktype[i] == 'true':
                    df = float(pla.cell(row=i + 2, column=7).value)
                    if df == 1:
    ...
    def userIdList(self,excel_path):
        ...
        for i in range(2, planum + 2):
            try:
                df = pla.cell(row=i, column=2).value.split(',')
